...
-- Grant Cortex Analyst roles the needed access for Snowsight (usage + select on the semantic view)
GRANT USAGE ON DATABASE EVO_DEMO TO ROLE SNOWFLAKE.CORTEX_USER;
GRANT USAGE ON SCHEMA IOWA_LIQUOR_SALES TO ROLE SNOWFLAKE.CORTEX_USER;
GRANT SELECT ON SEMANTIC VIEW SV_IOWA_LIQUOR_SALES TO ROLE SNOWFLAKE.CORTEX_USER;
GRANT USAGE ON DATABASE EVO_DEMO TO ROLE SNOWFLAKE.CORTEX_ANALYST_USER;
GRANT USAGE ON SCHEMA IOWA_LIQUOR_SALES TO ROLE SNOWFLAKE.CORTEX_ANALYST_USER;
GRANT SELECT ON SEMANTIC VIEW SV_IOWA_LIQUOR_SALES TO ROLE SNOWFLAKE.CORTEX_ANALYST_USER;
